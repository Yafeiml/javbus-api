...
export function parseFilterInfo(pageHTML: string, type: FilterType, value: string) {
    const doc = parse(pageHTML);
    const name =
        doc.querySelector('title')?.textContent.match(/^(?:\d+?頁 - )?(.+?) - /)?.[1] ?? '';
...
export async function getMovieDetail(id: string): Promise<MovieDetail> {
    ...
    const date = textInfoFinder(infoNodes, '発売日:');
    const videoLength = textInfoFinder(infoNodes, '収録時間:', '分');
    const numberVideoLength = videoLength ? Number(videoLength) : null;
    const director = linkInfoFinder(infoNodes, '監督:', 'director'); //导演
    const producer = linkInfoFinder(infoNodes, 'メーカー:', 'studio'); //发行
    const publisher = linkInfoFinder(infoNodes, 'レーベル:', 'label'); //系列
    const series = linkInfoFinder(infoNodes, 'シリーズ:', 'series'); //分类
